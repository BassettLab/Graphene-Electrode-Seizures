...
%--------------------------------------------------------------------------
llim = 350;  % <-- This threshold is chosen to be specific to ictal transition  
gflt = 15;   % <-- width of the gaussian image filter, arbitrary

clear binm edgm
for t = 1:size(ttif,1)
    if mod(t,250) == 0, disp(['Processed image ' num2str(t) ' of ' num2str(samps) ' - ' datestr(datetime(now,'ConvertFrom','datenum'))]);   end
    SM = imgaussfilt(squeeze(ttif(t,:,:)), gflt); 
